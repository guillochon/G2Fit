...
def obj_func(x, times, types, measurements, errors, objects, coords, prior, mode):
	...
	for i, stimes in enumerate(times):
		...
		if types[i] == 'pxy':
			...
			if (gi == 1):
				...
				if mode == 1:
					val += np.sum(np.log(np.sqrt((errors[i][:,0]*mhz)**2 + variance**2)))
					val += np.sum(np.log(np.sqrt((errors[i][:,1]*mhz)**2 + variance**2)))
			else:
				val += np.sum((((dat[:,0] - measurements[i][:,0]*mhz)/(errors[i][:,0]*mhz)))**2)
				val += np.sum((((dat[:,1] - measurements[i][:,1]*mhz)/(errors[i][:,1]*mhz)))**2)
		elif types[i] == 'vz':
			...
			dat[:,1] += mhvy[ci]*km
			vecs[i] = np.copy(dat)
			val += np.sum(((dat[:,0] - measurements[i][:,0])/errors[i][:,0])**2)
			val += np.sum(((dat[:,1] - measurements[i][:,1])/errors[i][:,1])**2)
		else:
...
#g2data[:,3:5] = g2data[:,3:5] + 100.*au/(8.*kpc)
...
# Adding some positional uncertainty to G2
#g2data2[:,3:5] = g2data2[:,3:5] + 100.*au/(8.*kpc)
...
# Just S2 with wide guess, no unused free parameters, includes variable variance
#x0 = em.utils.sample_ball([39.9595606861, 2.43751259014e+17, 0.00336573644595, 0.00189323680646, -4.44312540168, -1.71856390183, 32.0694144443, 2.64426936195e+22,
#						   1.576992696e+16, -0.907088055947, 225.398067805, 0.631748105165, 63.0430664828, 44.6995421214
#						   ],
#				  		  [0.1,1.0e15,0.0001,0.0001,20.,20.,20.,0.1*kpc,
#						   0.0005*pc,1.0,1.8,0.1,1.8,1.8
#						   ], size=nwalkers)
...
sampler = em.EnsembleSampler(nwalkers, ndim, obj_func, args = [times, types, measurements, errors, objects, coords, True, 1], pool = pool)
...
pos = x0
prob = np.array([obj_func(x, times, types, measurements, errors, objects, coords, True, 1) for x in x0])
...
for t, result in enumerate(sampler.sample(pos, iterations=nsteps, storechain=False, lnprob0=prob, rstate0=state)):
	...
	print best_prob
	best_prob = obj_func(best_pos, times, types, measurements, errors, objects, coords, False, 0)
	print best_prob
	lmh = best_pos[0]
	mh = 10.**lmh
...
if pool.is_master():
	...
	obj_func(pos[prob.argmax()], times, types, measurements, errors, objects, coords, False, 0)
	...
	for w in xrange(nwalkers):
		obj_func(pos[w], times, types, measurements, errors, objects, coords, False, 2)
		lmh = pos[w,0]
		kes = get_star_ke(elements, lmh)
		for i, e in enumerate(elements):
	...
	#fig, (posplt, velzplt, velxyplt) = plt.subplots(3)
	...
	#x = mhz*g2data[:,1]/mpc
	#y = mhz*g2data[:,2]/mpc
	#u = np.sqrt((mhz*g2data[:,3])**2 + variance**2)/mpc
	#v = np.sqrt((mhz*g2data[:,4])**2 + variance**2)/mpc
	...
	#u = s35vxydata[:,3]/km
	#v = s35vxydata[:,4]/km
	##vx0 = mhz*s35data[:,1]/mpc
	##vy0 = mhz*s35data[:,2]/mpc

	#velxyplt.errorbar(x, y, xerr=u, yerr=v, elinewidth=1, linewidth=0, fmt='y')
	#tbase = 10.*yr
	#ells = [Ellipse(xy=[vx0[i] + tbase*x[i]*km/mpc, vy0[i] + tbase*y[i]*km/mpc], width=6.*tbase*u[i]*km/mpc, height=6.*tbase*v[i]*km/mpc) for i in xrange(len(s35vxydata))]
	#for e in ells:
	#	posplt.add_artist(e)
	#	e.set_facecolor('y')
